...
(function ($) {
    ...
    var feature = {};
    feature.fileapi = $("<input id='target-stl-lib' type='file'/>").get(0).files !== undefined;
	...
	this.enqueueFile = function(file){
        ...
        function createCustomInputFile (obj, group, s, uploadLabel) {
            ...
            var fileUploadId = "ajax-upload-id-target";
            var form = $("<form id='ajax-stl-target-form' class='ajax-stl-target-form-class' method='" + s.method + "' action='" + s.url + "' enctype='" + s.enctype + "'></form>");
            var fileInputStr = "<input id='"+fileUploadId+"' class='ajax-stl-target' type='file' name='" + s.fileName + "' accept='" + s.acceptFiles + "'/>";
            if(s.multiple) {
                if(s.fileName.indexOf("[]") != s.fileName.length - 2) // if it does not endwith
                {
                    s.fileName += "[]";
                }
                fileInputStr = "<input id='"+fileUploadId+"' class='ajax-stl-target' type='file' name='" + s.fileName + "' accept='" + s.acceptFiles + "' multiple/>";
            }
